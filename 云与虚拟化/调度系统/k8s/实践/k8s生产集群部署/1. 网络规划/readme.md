...
* 云底层网络: 运行云的基础设施和辅助组件的网络，L(云管理网络)
...
* 用类似挂载浮动ip的方式将给的ip在一个网络中暴露(SNAT + DNAT)，该网络是客户端可以访问的网络。
* 类似浮动ip的方式有个缺陷，就是映射是1对1的，如果希望有一个ip代理多台多个主机，则可以使用类似lvs/nginx的代理服务器的方式完成
* 代理服务器的缺陷是需要主机具备多网卡，如果代理服务器不具有多网卡，最终还是要从所在网络的网关在向外映射一个地址，然后再将这个地址对外暴露
