...
云在vpc内也有一些探针和监控程序部署，用来实现诸如云监控等功能
(租户在页面上配置要检测的服务的vpc内ip和端口，定义监控数据指标和内容，最终呈现报表或发送告警。
这些探针需要向控制节点报告采集到的数据。

那么这些采集器直接连接控制节点的ip自然不是个好主意，云监控的团队可以通过公共网络开放一个浮动ip作为采集数据的汇报端点使。
这个公共网络的浮动ip在后面怎么又透过层层dnat转发到真正的控制节点则是另一个话题，在本文中不做展开

# 总结

公共网络的存在和所有连接到这个网络的路由器使得所有租户能够访问云厂商提供的一系列托管服务，也给云产研的非iaas开发团队提供了利用iaas资源同时又不干扰租户的能力；
当然，其它开发团队也可以采用自己习惯的开发模式，总之最终将对租户暴露的ip通过路由和地址映射搞到公共网络内即；
最终，公共网络可以作为云的控制节点的接口或者云的api与云内vpc之间交换数据的缓冲区，这个缓冲区作用很大，其中一个就是使得vpc内的云主机可以不通过浮动ip就访问云的api，
当然前提是api要在公共网络上暴露自己的一个服务ip。