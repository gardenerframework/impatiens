...
Boundary(块存储网络, 块存储网络, vlan-c) {
    Boundary(存储网网关, 存储网网关网络, vlan-c) {
        Container(10.2.254.253, 10.2.254.253, 文件存储访问网关) #green
    }
    System(交换机, 路由) #green
...
100.72.0.5  <-r-> 交换机
10.2.254.253 <-d-> 交换机
10.2.254.253 <-r-> 10.2.0.9

@enduml
```

在这方案内，nfs物理机地址假设是10.2.0.9，在机房的vlan号为vlan-c

* 在云内规划一个共享网络，vlan号为vlan-c，可用ip地址不妨使用存储网络最后几个可用ip, 比如从10.2.254.x。
  按照前面的规划，10.2.0.0/16为存储网子网，因此这个ip没有跨子网的问题，属于2层可以直接通信
* 在该vlan共享网络中创建一个router，不妨假设ip被分为10.2.254.253
* 该router联入100.72.0.0/24，并分配一个fip为100.72.0.5，该ip dnat到10.2.0.9
* 要访问nfs的vpc在100.64中的路由为100.64.0.100，在访问nfs时，获得的解析地址为100.72.0.5
* 按照三层路由规则，172.16.0.7 -> 172.16.0.1 -> snat 100.64.0.100 -> 
  100.64.0.1 -> 100.72.0.60 -> 100.72.0.5 -> dnat 10.2.0.9  -> 10.2.0.5 -> 10.2.0.9
